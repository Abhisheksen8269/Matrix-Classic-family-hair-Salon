...
const DATA_DIR = process.env.VERCEL ? '/tmp' : path.join(ROOT_DIR, 'data');
const BOOKINGS_FILE = path.join(DATA_DIR, 'bookings.json');
const REVIEWS_FILE = path.join(DATA_DIR, 'reviews.json');

// Ensure data files are seeded into /tmp when running on Vercel
if (process.env.VERCEL) {
    const srcBookings = path.join(ROOT_DIR, 'data', 'bookings.json');
    const srcReviews = path.join(ROOT_DIR, 'data', 'reviews.json');
    try {
...
const requestHandler = (req, res) => {
    ...
    // --- STATIC FILE SERVING (For local node execution) ---
